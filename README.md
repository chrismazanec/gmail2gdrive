...
* driveFolder (String, mandatory): a path to an existing Google Drive folder (will be created, if not existing)
* archive (boolean, optional): Should the gmail thread be archived after processing? (default: false)
* filenameFilter (String, optional): The attachment filename that should be renamed when stored in Google Drive
* filenameFilterRegexp (String, optional): A regular expression to specify only relevant attachments
* driveFilename (String, optional): The pattern for the new filename of the attachment. If 'filenameFilter' is not given then this will be the new filename for all attachments.
